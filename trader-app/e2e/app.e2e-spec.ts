...
describe('Starting tests for trader-app', function() {
  ...
  it('website title should be trader-app', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('trader-app');
    })
  });
  ...
  it('navbar-brand should be trader-app',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('trader-app');
    });
  });
