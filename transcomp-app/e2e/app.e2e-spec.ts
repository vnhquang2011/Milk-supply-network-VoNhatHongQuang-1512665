...
describe('Starting tests for transcomp-app', function() {
  ...
  it('website title should be transcomp-app', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('transcomp-app');
    })
  });

  it('network-name should be milk-supply-network@0.0.4',() => {
    element(by.css('.network-name')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('milk-supply-network@0.0.4.bna');
    });
  });

  it('navbar-brand should be transcomp-app',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('transcomp-app');
    });
  });
